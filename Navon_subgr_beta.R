...
allresults_Navon<-subset(german_beta_EO, VP_Code!="OS12GRO173") #change back to all if exclusions eeg not needed,
...
summary(lm)
lm<-lm((vis_SACS_Lcon-vis_SACS_Linc)~AQ_AD_Score+AQ_SW_Score+AQ_SK_Score+AQ_CO_Score+AQ_IM_Score, data=allresults_Navon_AP) ##!
summary(lm)
...
ggplot(data=allresults_Navon,aes(x=AQ_Score, y=(vis_SACS_Lcon-vis_SACS_Linc)))+
  ...
  geom_point(aes(color=AP.0.RP.1.AP.,size=starting_age))+theme_classic()+labs(x="AQ_Score",
                                                            ...
                                                            title="loc vs. global visual (beta)", 
                                                            subtitle="with EEG exclusions")+
...
#beta (run in other script!)
lm<-lm((vis_SACS_Lcon-vis_SACS_Linc)~AQ_Score+`Crand_ T10`+`Lrand_ T7`, data=allresults_Navon_AP) ##only AQ, Crand no extra 
summary(lm)
lm<-lm(AQ_Score~`Crand_ T10`, data=allresults_Navon_AP) #correllated
summary(lm)
...
#Navon exclude cases (missudnerstood L/G=5)
allresults_Navon2<-subset(EO_beta_network, VP_Code!="OS12GRO173") #change back to all if exclusions eeg not needed,
#or change to other EEG_results table (eeg. delta)
allresults_Navon2<-subset(allresults_Navon2, VP_Code!="EL01KES159")
allresults_Navon2<-subset(allresults_Navon2, VP_Code!="YL24DRO161")
allresults_Navon2<-subset(allresults_Navon2, VP_Code!="RI15SON966")
allresults_Navon2<-subset(allresults_Navon2, VP_Code!="AZ17FRA106")
...
ggplot(data=allresults_Navon,aes(x=AQ_Score, y=(vis_SACS_Lcon-vis_SACS_Linc)))+
  ...
  geom_point(aes(color=AP.0.RP.1.AP.,size=starting_age))+theme_classic()+labs(x="AQ_Score",
                                                                              y= "vis_SACS_Lcon - vis_SACS_Linc", 
                                                                              title="loc vs. global visual (beta) ", 
                                                                              subtitle="without EEG exclusions")+
  coord_cartesian(ylim=c(-1,1),xlim=c(0,36))+
  theme(axis.text.x=element_text(color="black", size=10),
        axis.text.y=element_text(color="black", size=10),
        legend.title=element_text(color="black", size=10,face="bold"),
